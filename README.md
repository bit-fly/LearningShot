...
内部使用的浏览器插件：在进行网页在线培训时，一键抓取整页或框选局部内容，用公司内部 LLM 网关做归纳解读；遇到自测题目时可切换为"参考答案"模式。

> ⚠️ 本插件仅供内部使用，未上架商店。"参考答案"功能仅适用于**自测/练习性质**的题目，请勿用于正式计分考试或认证考核。
...
- **普通解读 / 题目解答**：手动切换的模式开关。题目解答模式会给出参考答案 + 简要说明，并在结果开头提示"仅供自测练习参考"。
